...
function ManagerObjective({
  ...
  const [selfAverage, setSelfAverage] = useState(null);
  ...
  const fetchAppraisal = async () => {
    try {
     ...
     // debugger;
      const resData = await resAppraisal.json();
      if (resData) {
  ...
  useEffect(() => {
    if (
      reporteeappraisal?.managerGoals?.length &&
      selfRating?.selfGoals?.length
    ) {
      ...
      const managerAvg = calculateAverageManager(
        reporteeappraisal.managerGoals
      );
  ...
  const handleChange = (index, key, value, section) => {
    ...
    setSelfAverage(
      calculateAverageSelf(reporteeappraisal.managerGoals, selfRating.selfGoals)
    );
  ...
  const calculateAverageSelf = (managerGoals = [], selfGoals = []) => {
    ...
    managerGoals.forEach((mg) => {
      ...
      if (achievement && rating) {
        totalWeighted += Number((achievement  * rating)/100);
        totalCount++;
      }
    });
            ...
            {areas.map((area, index) => (
              <tr key={index}>
                ...
                <td>
                  ...
                  <textarea
                    value={
                      reporteeappraisal.manager?.[index]?.performance || ""
                    }
                    onChange={(e) =>
                      handleChange(
                        index,
                ...
                <td>
                  ...
                  <textarea
                    value={
                      reporteeappraisal.manager?.[index]?.achievements || ""
                    }
                    onChange={(e) =>
                      handleChange(
                        index,
                ...
                <td>
                  ...
                  <textarea
                    value={
                      reporteeappraisal.manager?.[index]?.developments || ""
                    }
                    onChange={(e) =>
                      handleChange(
                        index,
                ...
                <td>
                  <div>
                    <input
                      ...
                      value={
                        reporteeappraisal.managerGoals?.[index]?.achievements ||
                        ""
                      }
                      onChange={(e) =>
                        handleChange(
                ...
                <td>
                  <input
                   type="number"
                    ...
                    value={
                      reporteeappraisal.managerGoals?.[index]?.rating || ""
                    }
                    onChange={(e) =>
                      handleChange(
                        index,
